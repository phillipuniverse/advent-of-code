import { parseLine, fillBoard, countOverlaps } from './solution.js';
...
test("Solves part1", () => {
  let board = fillBoard(['#1 @ 1,3: 4x4',
    ...
    '#3 @ 5,5: 2x2']);
  console.log(board);
  expect(board[3][3]).toEqual(2);
  expect(board[3][4]).toEqual(2);
  expect(board[4][3]).toEqual(2);
  expect(board[4][4]).toEqual(2);
...
test("Solves part1", () => {
  let board = fillBoard(['#1 @ 1,3: 4x4',
    '#2 @ 3,1: 4x4',
    '#3 @ 5,5: 2x2']);
  expect(countOverlaps(board)).toEqual(4);
});
